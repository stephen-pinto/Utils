...
namespace SPUtils.Core.v02.Services.Reflection.Mapper
{
    public class PropertyMapperBi<T1, T2> : Dictionary<string, string>
        where T1 : class
        where T2 : class
    {
        ...
        public PropertyMapperBi()
        {
            _propertyConfig = new Dictionary<string, IPropertyMapperConfigBase>();
        }

        public void Add<BType>(Expression<Func<BType>> expr1, Expression<Func<BType>> expr2)
        {
        ...
        public void Add<BType>(Expression<Func<T1, BType>> expr1, Expression<Func<T2, BType>> expr2)
        {
            //If UnaryExpression then get operand of Body else get direct Body as MemberExpression
            var memExprssn1 = (MemberExpression)((expr1.Body is MemberExpression) ? expr1.Body : ((UnaryExpression)expr1.Body).Operand);
            var memExprssn2 = (MemberExpression)((expr2.Body is MemberExpression) ? expr2.Body : ((UnaryExpression)expr2.Body).Operand);
        ...
        public void CopyChanges(T1 sourceObj, T2 destinationObj, bool useDefaults = false)
        {
            foreach (var kvp in this)
            {
                ...
                {
                    //Note: We have to the next 2 steps to handle late binding which 
                    //does not bound a type at the earlier execution stage
